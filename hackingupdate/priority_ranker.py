...
def fallback_rank_and_tag(article):
    ...
    # Elevate based on CISA KEV and EPSS threat intelligence
    reason_notes = []
    if article.get("is_cisa_kev"):
        if article.get("cisa_ransomware"):
            rank = max(rank, 9)
            reason_notes.append("CISA KEV: Known Ransomware Campaign")
        else:
            rank = max(rank, 8)
            reason_notes.append("CISA KEV: Actively Exploited in Wild")

    epss = float(article.get("epss_score") or 0.0)
    if epss >= 0.50:
        rank = max(rank, 7)
        reason_notes.append(f"High EPSS: {epss:.1%}")

    # If no tag is found, tag as infra if KEV or standard terms appear, else network
...
def rank_batch_with_llm(batch: list[dict]) -> list[dict]:
    ...
    # Prepare lightweight payload for LLM to rank, enriched with KEV and EPSS signals
    simplified_articles = []
    for art in batch:
        intel_notes = []
        if art.get("is_cisa_kev"):
            rw = " (Known Ransomware Campaign Use)" if art.get("cisa_ransomware") else ""
            intel_notes.append(f"[CISA KEV: ACTIVELY EXPLOITED IN WILD{rw}]")
        epss_val = float(art.get("epss_score") or 0.0)
        if epss_val >= 0.20:
            intel_notes.append(f"[EPSS Probability: {epss_val:.1%}]")
        intel_header = f"{' '.join(intel_notes)} " if intel_notes else ""

        raw_preview = art.get("content_text", "")[:400]
        content_preview = f"{intel_header}{raw_preview}" + ("..." if len(raw_preview) >= 400 else "")
...
1. Assign a priority rank from 1 to 10 (1 = low interest news, 10 = highly actionable vulnerability, zero-day, exploitation TTP, or tool release of critical interest to a professional pentester).
   Note: Articles labeled [CISA KEV] MUST be ranked at least 8 (or at least 9 if ransomware-associated).
    ...
    try:
        ...
        for art in batch:
            ...
            if art_id in rankings_map:
                ...
                final_batch.append({
                    "id": art_id,
                    "rank": _sanitize_llm_rank(raw.get("rank"), art),
...
def _sanitize_llm_rank(raw_rank, article: dict | None = None) -> int:
    """Clamp the LLM's claimed rank into the valid 1-10 range and enforce
    threat intelligence severity floors for CISA KEV and high EPSS.
    """
    try:
        rank = int(raw_rank)
    ...
    if article:
        if article.get("is_cisa_kev"):
            rank = max(rank, 9 if article.get("cisa_ransomware") else 8)
        elif float(article.get("epss_score") or 0.0) >= 0.50:
            rank = max(rank, 7)
...
def main():
    ...
    # Enrich articles with CISA KEV and EPSS threat intelligence before ranking
    try:
        from hackingupdate.cve_enrichment import enrich_articles
        articles = enrich_articles(articles)
    except Exception as e:
        logger.error(f"Error enriching articles with CVE threat intel: {e}")
