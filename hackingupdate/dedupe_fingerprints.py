...
def main():
    ...
    # 1. Cross-Day SQLite Deduplication: filter out articles already reported in recent days
    historical_links: set[str] = set()
    historical_titles: set[str] = set()
    cross_day_skipped = 0

    if config.ENABLE_CROSS_DAY_DEDUPE and config.CROSS_DAY_DEDUPE_DAYS > 0:
        try:
            from datetime import date
            import hackingupdate.db_manager as db_manager
            db_manager.init_db()
            historical_links, historical_titles = db_manager.get_recent_finding_identifiers(
                days=config.CROSS_DAY_DEDUPE_DAYS,
                exclude_date=date.today(),
            )
            if historical_links or historical_titles:
                logger.info(
                    f"Cross-day deduplication active: loaded {len(historical_links)} historical links "
                    f"and {len(historical_titles)} titles from the past {config.CROSS_DAY_DEDUPE_DAYS} days."
                )
        except Exception as e:
            logger.warning(f"Could not load historical findings for cross-day deduplication: {e}")

    articles_to_process = []
    for art in articles:
        link = art.get("link", "").strip()
        norm_title = " ".join(art.get("title", "").lower().split())
        if link and link in historical_links:
            logger.debug(f"Discarding cross-day duplicate link: {art.get('title')}")
            cross_day_skipped += 1
            continue
        if norm_title and norm_title in historical_titles:
            logger.debug(f"Discarding cross-day duplicate title: {art.get('title')}")
            cross_day_skipped += 1
            continue
        articles_to_process.append(art)

    if cross_day_skipped > 0:
        logger.info(f"Cross-day deduplication: filtered {cross_day_skipped} articles already covered in recent briefings.")

    # 2. Intra-run deduplication
    unique_articles = []
