...
def send_teams_notification(webhook_url, today_str, exec_summary, working_set):
    ...
    for art in top_vulnerabilities:
        ...
        badges = []
        if art.get("is_cisa_kev") or art.get("cisa_kev", {}).get("is_kev"):
            rw_str = " (Ransomware)" if (art.get("cisa_ransomware") or art.get("cisa_kev", {}).get("ransomware")) else ""
            badges.append(f"🚨 CISA KEV{rw_str}")
        epss_val = float(art.get("epss_score") or art.get("epss", {}).get("max_score") or 0.0)
        if epss_val >= 0.20:
            badges.append(f"EPSS {epss_val:.0%}")
        badge_prefix = f"[{' | '.join(badges)}] " if badges else ""

        facts.append({
            "name": f"{badge_prefix}⭐ {title} (Rank {rank}/10)",
