...
def load_working_set_with_fallback(today_str):
    ...
    try:
        from hackingupdate.db_manager import get_findings_by_date
...
def format_whatsapp_message(today_str, exec_summary, working_set):
    ...
    msg += "🔥 *Top Actionable Threats Today:*\n"
    ...
    for art in top_articles:
        ...
        link = art.get("link", "#")
        is_kev = bool(art.get("is_cisa_kev") or art.get("cisa_kev", {}).get("is_kev"))
        is_rw = bool(art.get("cisa_ransomware") or art.get("cisa_kev", {}).get("ransomware"))
        epss_val = float(art.get("epss_score") or art.get("epss", {}).get("max_score") or 0.0)

        badge_parts = []
        if is_kev:
            badge_parts.append("🚨 CISA KEV" + (" [Ransomware]" if is_rw else ""))
        if epss_val >= 0.30:
            badge_parts.append(f"EPSS: {epss_val:.0%}")

        badge_line = f"  [{' | '.join(badge_parts)}]\n" if badge_parts else ""

        art_block = f"\n• *{title}*\n"
        if badge_line:
            art_block += badge_line
