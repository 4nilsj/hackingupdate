...
from hackingupdate.report_formatting import (
    _safe_link,
    _safe_slug,
    _sanitize_html,
...
from hackingupdate.report_template import HTML_TEMPLATE
...
def parse_markdown_to_premium_html(md_path, today_str):
    ...
    # Load working set metadata for CISA KEV and EPSS threat intel if available
    working_intel: dict[str, dict] = {}
    if config.WORKING_CACHE_FILE.exists():
        try:
            with open(config.WORKING_CACHE_FILE, "r", encoding="utf-8") as f:
                ws_data = json.load(f)
                for item in ws_data:
                    item_link = item.get("link", "").strip().lower()
                    item_title = " ".join(item.get("title", "").lower().split())
                    if item_link:
                        working_intel[item_link] = item
                    if item_title:
                        working_intel[item_title] = item
        except Exception:
            pass
    ...
    for section in sections:
        ...
        for art in articles:
            ...
            # Threat intelligence signals (from working set or markdown body)
            kev_in_body = bool(re.search(r'CISA KEV.*?(?:Known Exploited|Actively Exploited)', art_body, re.IGNORECASE))
            rw_in_body = bool(re.search(r'Ransomware', art_body, re.IGNORECASE))
            epss_in_body_match = re.search(r'EPSS.*?:\s*`?([\d.]+)%?`?', art_body, re.IGNORECASE)

            ws_meta = working_intel.get(link.lower()) or working_intel.get(" ".join(art_title.lower().split()), {})
            is_kev = bool(ws_meta.get("is_cisa_kev") or kev_in_body)
            is_ransomware = bool(ws_meta.get("cisa_ransomware") or (is_kev and rw_in_body))
            epss_score = float(ws_meta.get("epss_score") or 0.0)
            if not epss_score and epss_in_body_match:
                try:
                    val = float(epss_in_body_match.group(1))
                    epss_score = val / 100.0 if val > 1.0 else val
                except Exception:
                    pass

            intel_badges = []
            if is_kev:
                intel_badges.append('<span class="cisa-kev-badge">🚨 CISA KEV</span>')
            if is_ransomware:
                intel_badges.append('<span class="ransomware-badge">⚠️ Ransomware</span>')
            if epss_score >= 0.15:
                intel_badges.append(f'<span class="epss-badge">EPSS: {epss_score:.0%}</span>')
            intel_badges_html = "".join(intel_badges)
            ...
            # Strip raw threat intel bullet lines from body since badges are in the header
            cleaned_art_body = re.sub(r'^\s*[-*]\s+\*\*CISA KEV[^\n]*\n?', '', cleaned_art_body, flags=re.MULTILINE | re.IGNORECASE)
            cleaned_art_body = re.sub(r'^\s*[-*]\s+\*\*EPSS[^\n]*\n?', '', cleaned_art_body, flags=re.MULTILINE | re.IGNORECASE)
            ...
            seen_articles[norm_key] = {
                ...
                "intel_badges_html": intel_badges_html,
