...
def main():
    ...
    # Select working set based on threshold: rank >= 5 OR confirmed CISA KEV
    working_set = [art for art in unique_ranked_articles if art.get("rank", 0) >= 5 or art.get("is_cisa_kev")]
    ...
    # Prioritize: Sort so that CISA KEV items appear with highest priority, then by rank descending
    working_set.sort(
        key=lambda x: (
            1 if x.get("is_cisa_kev") else 0,
            1 if x.get("cisa_ransomware") else 0,
            x.get("rank", 0),
            float(x.get("epss_score") or 0.0)
        ),
        reverse=True
    )
